...
export function SearchFormSection({onSearch, filters}) {
  const idText = useId();
  ...
  const {handleSubmit, searchText, setSearchText} = useSerchForm({idTechnology, idText, idLocation, idExperienceLevel, onSearch, filters})
  ...
  return (
    ...
    <form onSubmit = {handleSubmit} id='empleos-search-form'  role="search">
      <div>
          ...
          name={idText} value={searchText} onChange={(e) => setSearchText(e.target.value)} id="empleos-search-input" type="search" 
          placeholder="Buscar trabajos, empresas o habilidades"
          style = {{BorderColor: focusField ==="search" ? '#4f46e5' : '#d1d5db',
          outline: focusField === "search" ? "2px solid #4f46e5": "none" 
          }} />
          <button  id="" type="submit">Buscar</button>
      </div>
      {focusField === "search" ? 
      ...
      <div className="aling-left" role="filter">
        <select name= {idTechnology} defaultValue={filters.technology} id="search-tecnology">
        ...
        <select name={idLocation} defaultValue={filters.location} id="search-ubi">
        ...
        <select name={idExperienceLevel} defaultValue={filters.experienceLevel} id="search-nivel">
